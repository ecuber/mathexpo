...
import { Emoji, Float, Quiz, StaticEq } from '../../components/layout'
...
const HigherDimensions = props => {
  return <>
    ...
    <Center mb='4' style={{ fontSize: '1.7rem' }}><StaticEq value='2x+3y=6'/></Center>
    ...
    <Quiz placeholder={1}>
      ...
      <><Emoji symbol='🤔' label='thinking face'/> Not quite. The line is where <StaticEq inline value='2x+3y=6'/> is equal to 6, and on one side it will be greater and on the other side less.</>
      <><Emoji symbol='✅' label='green check mark'/> Exactly. The line is where <StaticEq inline value='2x+3y=6'/> is equal to 6, and on one side it will be greater and on the other side less.</>
    </Quiz>

    <p>So, mathematically, the search for a separating line is equivalent to a search for an expression like <StaticEq inline value='2x+3y'/> which happens to be larger than a certain value (6, say) for all teal points and smaller that value for all tomato-colored points.</p>
    <p>Likewise, the equation for a separating plane in three dimensions might be something that looks like:</p>
    <Center mb='4' style={{ fontSize: '1.7rem' }}><StaticEq value='2x+3y+6z=12'/></Center>
    <p>in which case the <StaticEq inline value='2x+3y+6z'/> value for each teal point would be greater than 12 and for each tomato point would be less than 12.</p>
    ...
    <Center mb='4' style={{ fontSize: '1.7rem' }}><StaticEq value='−w+3x−2y+4.5z=1'/></Center>
    <p>where the left-hand side is smaller for each point <StaticEq inline value='(w,\space x,\space y,\space z)'/> that corresponds to a pedestrian-containing image, and where the left-hand side is greater for the rest of the images.</p>
